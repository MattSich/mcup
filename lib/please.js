...
var PATH_TO_DEPS_SCRIPT = path.resolve(PATH_TO_SCRIPTS, 'rh-deps.sh');
var PATH_TO_NGINX_CONF = path.resolve(PATH_TO_CONFS, 'nginx.conf');
var PATH_TO_SYSTEMD_CONF = path.resolve(PATH_TO_CONFS, 'noded');
...
please.prototype.init = function () {
	...
	var PATH_TO_MPLZ_SETTINGS_JSON = path.resolve(this.current_dir, 'mcup.json');

	prompt.message = "[mcup.json]".magenta;
	prompt.start();
...
please.prototype.setup = function () {
	...
	if (this.config.setupNginx) {
		...
		taskList.execute('Opening up access to nginx dir...', {
			command: 'sudo chmod -R 777 /etc/nginx/'
		});
...
please.prototype.deploy = function () {
	...
	var bash = spawn("bash", [PATH_TO_BUILD_SCRIPT], {
		cwd: this.config.app
	});
	...
	bash.on('close', function (code) {
		if (code) {
			...
			taskList.execute('Opening up permissions...', {
				command: 'sudo chmod -R 777 /var/www/' + config_appName
			});
			...
			taskList.copy('Uploading modules...', {
				src: config_app + '/bundle/node_modules.tar.gz',
				dest: config_appRemoteTargetPath + '/bundle/programs/server/node_modules.tar.gz'
			});
			// run dependency unpack on server
			taskList.executeScript('Deploying modules...', {
				script: PATH_TO_DEPS_SCRIPT,
				vars: {
					appRemoteTargetPath: config_appRemoteTargetPath,
					appRootUrl: config_appRootUrl,
					appPort: config_appPort,
					appMongoUrl: config_env_MONGO_URL || ('mongodb://127.0.0.1:27017/' + this.config.appName),
					appUser: config_appUser,
					appName: config_appName,
...
please.prototype.reconfig = function () {
	...
	taskList.execute('Opening up permissions...', {
		command: 'sudo chmod -R 777 /etc/init.d/'
	});
	taskList.copy('Reconfiguring system daemon...', {
		src: PATH_TO_SYSTEMD_CONF,
		dest: '/etc/init.d/' + this.config.appName,
		vars: {
			appRootUrl: config_appRootUrl,
			appPort: config_appPort,
	...
	// reload systemd
	taskList.execute('Starting app service... ', {
		command: 'sudo service ' + this.config.appName + ' start'
	});
	// send restart to nginx daemon
	taskList.execute('Restarting nginx... ', {
		command: 'sudo service nginx restart'
...
please.prototype.stop = function () {
	...
	taskList.execute('Stopping ' + this.config.appName + ' daemon...', {
		command: 'sudo service ' + this.config.appName + ' stop'
	});
...
please.prototype.restart = function () {
	...
	taskList.execute('Restarting ' + this.config.appName + ' daemon...', {
		command: 'sudo service ' + this.config.appName + ' restart'
	});
	// send restart to nginx daemon
	taskList.execute('Restarting nginx... ', {
		command: 'sudo service nginx restart'
...
please.prototype.delete = function () {
	...
	taskList.execute('Removing daemon...', {
		command: 'sudo rm -rf /etc/init.d/' + this.config.appName
	});
	// send restart to nginx daemon
	taskList.execute('Restarting nginx... ', {
		command: 'sudo service nginx restart '
